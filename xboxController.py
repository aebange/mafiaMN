...
def sample_first_joystick():
    ...
    # Assigns device numbers to the items in the list of connected devices
    device_numbers = list(map(attrgetter('device_number'), joysticks))

    controller_key_press = 0

    # print('found %d devices: %s' % (len(joysticks), device_numbers))

    # If no devices are connected, crash the program
    if not joysticks:
        sys.exit(0)

    # Assign the first connected device to j
    j = joysticks[0]
    ...
    @j.event
    def on_button(button, pressed):
        nonlocal controller_key_press
        # When a button is pressed, set nonlocal controller_key_press to the number of the button pressed, and 1 representing a button rather than 0 (a stick)
        if pressed == 1:
            controller_key_press = [button,1]
    ...
    def on_axis(axis, value):
        ...
        if axis == "l_thumb_y":
            if value > .25:
                # The left stick is pushed up more than halfway, return the stick number (fake, set to 66 by myself) and 0 to indicate its a stick
                controller_key_press = [66, 0]
            if value < (-.25):
                # The left stick is pushed down more than halfway, return the stick number (fake, set to 66 by myself) and 0 to indicate its a stick
                controller_key_press = [65, 0]
